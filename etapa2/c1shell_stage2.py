...
GT            = 15     # '>' (new token)
LT            = 16     # '<' (new token)
BEGINBLOCK    = 17     # '[' (new token)
ENDBLOCK      = 18     # ']' (new token)
STRING        = 19     # string literal (new token)
...
smalltokens = {'=':ASSIGNOP, '(':LEFTPAREN, ')':RIGHTPAREN,
               '+':PLUS, '-':MINUS, '*':TIMES, '\n':NEWLINE, '':EOF,
               '^':POWER, '/':DIV, '>':GT, '<':LT, '[':BEGINBLOCK, ']':ENDBLOCK}
...
def tokenizer():
   ...
   while True:
      ...
      elif curchar.isdigit():            # start of unsigned int?
...
def ifstmt():
   ...
   consume(BEGINBLOCK)  # '['
   consume(NEWLINE)
   simplestmt()  # Statement inside block
   consume(NEWLINE)
   consume(ENDBLOCK)  # ']'
   # Optional NEWLINE
...
def condition():
   # First operand (UNSIGNEDINT or NAME)
   if token.category not in [UNSIGNEDINT, NAME]:
      raise RuntimeError('Expecting unsigned integer or variable in condition')
   advance()
   # Comparator (< or >)
   if token.category not in [LT, GT]:
      raise RuntimeError('Expecting < or > in condition')
   advance()
   # Second operand (UNSIGNEDINT or NAME)
   if token.category not in [UNSIGNEDINT, NAME]:
      raise RuntimeError('Expecting unsigned integer or variable in condition')
   advance()
...
def printstmt():
   ...
   consume(LEFTPAREN)
   # Handle either string or expression
...
def factor():
   if token.category == PLUS:
      ...
      expr()
      consume(RIGHTPAREN)
   elif token.category == DIV:
      advance()
      factor()
   elif token.category == POWER:
      advance()
      factor()
   else:
